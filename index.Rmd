...
num<-15
guess<-if(select==1){
        paste("The number is smaller then ",num)
      } else
        if(select==2){
          paste("The number is greater then ",num)
        } else
          paste("The number is ",num)

check<-if(select==1){
        ifelse(x < num,"That's true, go on.","That's not correct.")
      } else
        if(select==2){
          ifelse(x > num,"That's true, go on.","That's not correct.")
        } else
          ifelse(x==num,"Congratulations, you won.","Try again.")
guess
check
```
